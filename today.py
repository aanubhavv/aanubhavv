...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    # Filter out repositories with incomplete data - fix the None check
...
def flush_cache(edges, filename, comment_size):
    ...
    with open(filename, 'w') as f:
        f.writelines(data)
        for node in edges:
            # Check if node and nameWithOwner exist before processing
            if node.get('node') and node['node'].get('nameWithOwner'):
                f.write(hashlib.sha256(node['node']['nameWithOwner'].encode('utf-8')).hexdigest() + ' 0 0 0 0\n')
            else:
                # Skip repositories that don't have proper data
                print(f"Warning: Skipping repository with incomplete data: {node}")
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    ...
    root = tree.getroot()
    # Add this line to update the age_data (Uptime) field
...
if __name__ == '__main__':
    """
    Anubhav Garg (aanubhavv), 2022-2025
    """
    print('Calculation times:')
    # define global variable for owner ID and calculate user's creation date
    # e.g {'id': 'U_kgDOBwJEWw'} and 2022-11-06T16:23:57Z for username 'aanubhavv'
